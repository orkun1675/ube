...
// shared nano store via `openRequestAccess('maintainer_final_cta')` —
// preserving the analytics source label fired by `request_access_modal_opened`.
...
for (const btn of ctaButtons) {
  btn.addEventListener("click", () => {
    openRequestAccess("maintainer_final_cta")
  })
}
