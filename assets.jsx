...
const Logo = ({ children, size = 20, bg = null, rounded = 6, style = {} }) => (
  <span
    style={{
      display: "inline-flex",
      alignItems: "center",
...
// React Native — atom
const ReactLogo = ({ size = 20, color = "#61DAFB" }) => (
...
const MailIconFilled = ({ size = 16, color = "currentColor" }) => (
  <svg width={size} height={size * 0.75} viewBox="0 0 24 18" fill={color} aria-hidden="true">
...
// LinkedIn
const LinkedInLogo = ({ size = 18, color = "currentColor" }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill={color} aria-hidden="true">
...
// Error / circle x
const ErrorIcon = ({ size = 14, color = "currentColor" }) => (
...
// Warning
const WarnIcon = ({ size = 14, color = "currentColor" }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
const Glyph = ({ paths, size = 20, color = "currentColor" }) => (
...
const FlaskIcon  = ({ size = 20 }) => <Glyph size={size} paths={<><path d="M9 3h6M10 3v6L4 19a2 2 0 0 0 1.7 3h12.6A2 2 0 0 0 20 19L14 9V3"/><path d="M7 14h10"/></>}/>;
const StackIcon  = ({ size = 20 }) => <Glyph size={size} paths={<><rect x="3" y="4" width="18" height="5" rx="1.5"/><rect x="3" y="11" width="18" height="5" rx="1.5"/><path d="M7 17v3M12 17v3M17 17v3"/></>}/>;
const ClockIcon  = ({ size = 20 }) => <Glyph size={size} paths={<><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></>}/>;

// Benefit icons
const SparkIcon  = ({ size = 20 }) => <Glyph size={size} paths=<path d="M12 3v4M12 17v4M3 12h4M17 12h4M5.6 5.6l2.8 2.8M15.6 15.6l2.8 2.8M5.6 18.4l2.8-2.8M15.6 8.4l2.8-2.8"/>/>;
const ChatIcon   = ({ size = 20 }) => <Glyph size={size} paths={<><path d="M21 12a8 8 0 1 1-3-6.2L21 4l-1 4.2A8 8 0 0 1 21 12Z"/><circle cx="9" cy="12" r="0.8" fill="currentColor"/><circle cx="13" cy="12" r="0.8" fill="currentColor"/><circle cx="17" cy="12" r="0.8" fill="currentColor"/></>}/>;
const ShieldIcon = ({ size = 20 }) => <Glyph size={size} paths={<><path d="M12 3l8 3v6c0 5-3.5 8.5-8 9-4.5-.5-8-4-8-9V6l8-3Z"/><path d="M9 12l2 2 4-4"/></>}/>;
const TrendIcon  = ({ size = 20 }) => <Glyph size={size} paths={<><path d="M3 17l5-5 4 4 9-9"/><path d="M14 7h7v7"/></>}/>;
const StarIcon   = ({ size = 20 }) => <Glyph size={size} paths=<path d="M12 3l2.7 5.7 6.3.9-4.6 4.4 1.1 6.2L12 17.3 6.5 20.2l1.1-6.2L3 9.6l6.3-.9L12 3Z"/>/>;
const RocketIcon = ({ size = 20 }) => <Glyph size={size} paths={<><path d="M14 4c4 0 6 2 6 6-1 4-5 8-9 9-2-1-3-2-4-4 1-4 5-8 9-9-1 2-2 3-2 5Z"/><circle cx="14" cy="10" r="1.2" fill="currentColor"/><path d="M7 17c-2 1-2 2-3 4 2-1 3-1 4-3"/></>}/>;

// Lightning / inbox / list / fix / report icons used in step indicator pills
const InboxIcon = ({ size = 16 }) => <Glyph size={size} paths={<><path d="M3 13l3-9h12l3 9"/><path d="M3 13v6a1 1 0 0 0 1 1h16a1 1 0 0 0 1-1v-6h-6a3 3 0 0 1-6 0H3Z"/></>}/>;
const ListIcon  = ({ size = 16 }) => <Glyph size={size} paths={<><path d="M8 5h13M8 12h13M8 19h13"/><circle cx="4" cy="5" r="1"/><circle cx="4" cy="12" r="1"/><circle cx="4" cy="19" r="1"/></>}/>;
const WrenchIcon = ({ size = 16 }) => <Glyph size={size} paths=<path d="M21 7a4 4 0 0 1-5 5l-7 7-3-3 7-7a4 4 0 0 1 5-5l-2 3 2 2 3-2Z"/>/>;
const BellIcon = ({ size = 16 }) => <Glyph size={size} paths={<><path d="M6 16V11a6 6 0 0 1 12 0v5l2 2H4l2-2Z"/><path d="M10 19a2 2 0 0 0 4 0"/></>}/>;
const LinkIcon = ({ size = 16 }) => <Glyph size={size} paths={<><path d="M10 14a4 4 0 0 0 5.5 0l3-3a4 4 0 1 0-5.5-5.5L11 7"/><path d="M14 10a4 4 0 0 0-5.5 0l-3 3a4 4 0 1 0 5.5 5.5L13 17"/></>}/>;

Object.assign(window, {
  Logo, ImgLogo,
  GitHubLogo, SentryLogo, CrashlyticsLogo, PlayLogo, AppStoreLogo,
  ActionsLogo, AppleLogo, AndroidLogo, ExpoLogo, ReactLogo, FlutterLogo,
  MailIcon, XLogo, LinkedInLogo, PlusIcon, ArrowRight, CheckIcon, ErrorIcon, WarnIcon,
  MailIconFilled,
  Glyph, FlaskIcon, StackIcon, ClockIcon,
  SparkIcon, ChatIcon, ShieldIcon, TrendIcon, StarIcon, RocketIcon,
  InboxIcon, ListIcon, WrenchIcon, BellIcon, LinkIcon,
});
